...
export const createCrudActions = (actionTypes, uri, overrides = {}) => ({
	...
	create: data => ({
		...
		payload: axios.post(`/api/v1/${uri}`, data),
	}),
	update: (id, data) => ({
		type: actionTypes.update,
		meta: { affectsAll: false, arrayAction: 'update', timestamp: Date.now() },
		payload: axios.put(`/api/v1/${uri}/${id}`, data),
	}),
	delete: id => ({
 ...
 */
export const createCrudReducer = (actionTypes, initialState = initialCrudState, overrides = {}) => (
	state = initialState,
	action
) => {
	...
	Object.values(actionTypes).forEach(type => {
		// Request started
		reducerMap[`${type}_${ASYNC_SUFFIXES.loading}`] = (state, _) => ({
			...state,
			fetching: true,
			fetched: false,
		});
		...
		reducerMap[`${type}_${ASYNC_SUFFIXES.success}`] = (state, action) =>
			produce(state, draft => {
				// Update status
					...
					switch (action.meta.arrayAction) {
						...
						case 'update':
							...
							if (idx === -1)
								...
								draft.data.push(action.payload.data);
							// Else modify
							else draft.data[idx] = action.payload.data;
							break;
						case 'delete':
						default:
							// Get the index of the resource in the array
		...
		// Request failed
		reducerMap[`${type}_${ASYNC_SUFFIXES.error}`] = (state, action) => ({
			...state,
			fetching: false,
			fetched: false,
	...
	if (overrides.hasOwnProperty(action.type)) return overrides[action.type](state, action);
	// Sinon s'il s'agit d'une action CRUD, utiliser celle de reducerMap
	if (reducerMap.hasOwnProperty(action.type)) return reducerMap[action.type](state, action);
	// Sinon rien
	return state;
};
